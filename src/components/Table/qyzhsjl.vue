<template>
  <div>
    <div class="box" id="qyzhsfl">
      ...
      <el-table :data="tablemsgmsg" :style="style" class="xiazai" v-show="show">
        <el-table-column
          label="序号"
          type="index"
          width="50"
        ...
        <el-table-column
          ...
          min-width="40px"
        >
          <template slot-scope="scope">{{ scope.row.rate.split("%")[0]}} {{fuhao}}</template>
        </el-table-column>
        <el-table-column
          prop="remark"
          label="备注"
...
export default {
  ...
  mounted() {
    ...
    // 通过区域公司id和当前的年份来查询数据
    this.axios
      .get("/api/proChargeRate10?companyId=" + this.qyid + "&year=" + this.year)
      .then(res => {
  ...
  methods: {
    ...
    yearchange(yeardefaultdefault) {
      ...
      this.axios
        .get(
          "/api/proChargeRate10?companyId=" + this.qyid + "&year=" + this.year
        )
        .then(res => {
          // console.log(res.data.data)
          this.tablemsgmsg = res.data.data; //下载数据的表格
