<template>
  <div>
    <div class="box" id="httz">
      <div class="boxTitle">
        ...
        <div class="queryParent">
          <el-input
            ...
            v-model="tableDataEnd"
            style="display: inline-block;width: 200px;"
            :class="isShow === true ? 'info' : 'success'"
        ...
        <h2
          @click="exportExcel"
          class="daochu"
          :class="isShow === true ? 'info1' : 'success1'"
        >
          导出
        </h2>
      </div>
      ...
      <el-table
        :data="
          tablemsg.slice((currentPage - 1) * pagesize, currentPage * pagesize)
        "
        :style="style"
...
export default {
  ...
  mounted() {
    ...
    this.axios
      .get("/api/userToProject?staffId=10000&projectId=" + this.xmid)
      .then(res => {
        // console.log(res.data.data)
  ...
  methods: {
    ...
    Backstage() {
      window.location.href =
        this.url +
        "jkData/data/daq/projectContract/index?loginName=" +
        this.login +
        "&token=1";
    },
