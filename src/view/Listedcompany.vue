...
export default {
  ...
  methods: {
    ...
    chickvalue(msg) {
      ...
      this.axios
        .get(
          "/api/listedCompany09?reportingType=" + id + "&reportingYear=" + year
        )
        .then((res) => {
          // 再次赋值替换原数据
    ...
    getindex(value) {
      ...
      let id = encodeURI(value.slice(4));
      this.axios
        .get(
        ...
        .then((res) => {
          ...
          this.srqk = obj.sort(compare("generalIncome"));
        });
    },
  },
  mounted() {
    ...
    this.axios.get("/api/listedCompany10").then((res) => {
      ...
      this.axios
        .get(
          "/api/listedCompany09?reportingType=" +
            this.id +
            "&reportingYear=" +
            this.year
        )
...
.container_left_header .el-table .cell,
.el-table th div,
.el-table--border td:first-child .cell,
.el-table--border th:first-child .cell {
  padding: 5px;
