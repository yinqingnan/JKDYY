            ...
            <div class="center">
                ...
                <div class="left_header">
                    ...
                    <div>
                        <div>
                            <h1>高层</h1>
                            <h2>{{defaultdata.highRiseFee | number}}元/m²</h2>
                        </div>
                        <div>
                            <h1>洋房</h1>
                            <h2>{{defaultdata.westernFee | number}}元/m²</h2>
                        </div>
                        <div>
                            <h1>别墅</h1>
                            <h2>{{defaultdata.villaFee | number}}元/m²</h2>
                        </div>
                        <div>
                            <h1>商业</h1>
                            <h2>{{defaultdata.businessFee | number}}元/m²</h2>
                        </div>
                        <div>
                            <h1>车库</h1>
                            <h2>{{defaultdata.garageFee | number}}元/个</h2>
                        </div>
                    </div>
                ...
                <div class="center_body">
                    ...
                    <div v-if="xs">
                        <div class="right">
                            <ul class="right_body">
                                <li v-for="(item,index) in defaultlist"  v-if="index<5" :key="index"  >
            ...
            <div class="left">
                ...
                <div class="left_body">
                    ...
                    <div class="left_body_header">
                        <!-- table1 -->
                        <h1 class="titlezt">金科自建物业费备案价</h1>
                        <el-table
                            border
                            :data="defaultable1"
...
export default {
    ...
    mounted(){
        ...
        //请求默认显示的初始值id和城市列表
        axios.get("/api/cityData03?provinceName="+encodeURI(this.cityname))          
        .then((res)=>{
           ...
           this.citylist=res.data.data            //下拉菜单数据
            ...
            if(res.data.data[0]===undefined){
                ...
                axios.get("/api/cityData04?id="+res.data.data[0].id) 
                .then((res)=>{
                    // console.log(res)
                this.defaultdata=res.data.data[0]
                })
                    // 获取金科自建物业费备案价  table1
                axios.get("/api/cityData05?id="+res.data.data[0].id)
                .then((res=>{
                    this.defaultable1=res.data.data
                }))
                // 获取竞品项目物业费备案价 table2
                axios.get("/api/cityData06?cityDataId="+res.data.data[0].id)
                .then(res=>{
                    // console.log(res.data.data)
                    this.defaultable2=res.data.data
                })
                //获取物业费政府指导价
                axios.get("/api/cityData08?cityDataId="+res.data.data[0].id)
                .then((res)=>{
                    // console.log(res.data.data)
                    this.defaultable3=res.data.data
                })
                // 关键政策解读
                axios.get("/api/cityData07?cityDataId="+res.data.data[0].id)
                .then((res)=>{
        ...
        $route:{            
            ...
            handler(newVal){
                this.cityname=newVal.fullPath.slice(6)
                 axios.get("/api/cityData03?provinceName="+encodeURI(newVal.fullPath.slice(6)))
                    .then((res)=>{
    ...
    methods: {
        ...
        // 获取物业指导价数据
        axios.get("/api/cityData04?id="+name)
        .then((res)=>{
        ...
        // 获取金科自建物业费备案价  table1
        axios.get("/api/cityData05?id="+name)
        .then((res=>{
            this.defaultable1=res.data.data
        ...
        // 获取竞品项目物业费备案价 table2
        axios.get("/api/cityData06?cityDataId="+name)
        .then(res=>{
            // console.log(res.data.data)
            this.defaultable2=res.data.data
        })
        ...
        //获取物业费政府指导价
        axios.get("/api/cityData08?cityDataId="+name)
        .then((res)=>{
            // console.log(res.data.data)
            this.defaultable3=res.data.data
        })
        // 关键政策解读
        axios.get("/api/cityData07?cityDataId="+name)
        .then((res)=>{
            this.defaultlist=res.data.data
            // console.log(res.data.data)
        })
