        ...
        <div class="container_body">
          <div class="container_body_left">
            <div class="container_body_left_header">
              <div>
                ...
                <div>
                  <ul>
                    <li>官方网址:</li>&nbsp;
                    <li
                      @click="open(data.officialwebsite)"
                      style="cursor: pointer;color:#007af4;text-decoration:underline; "
                      :title="data.officialwebsite"
                    >{{data.officialwebsite}}</li>
                  </ul>
                  <ul>
                    <li>上市时间:</li>&nbsp;
              ...
              <ul>
                <li>
                  <h2 style="width:56px">上市地:</h2>
                  <h2>{{data.listingPlace}}</h2>
                </li>
                <li>
                  <h2>
                    总部地址:
                  ...
                  <h2>{{data.headquartersAddress}}</h2>
                </li>
              </ul>
            </div>
...
export default {
    ...
    $route: {
      handler(newVal) {
        ...
        axios.get("/api/listedCompany03?id=" + newVal.query.id).then(res => {
          ...
          this.glgmbn = res.data.data.filter(item =>
            item.reportingType.indexOf("全年")
          ); //半年数据
          this.GLGM = this.glgmbn; //默认显示半年数据
        });
        // 请求总收入图表数据
        axios.get("/api/listedCompany05?id=" + newVal.query.id).then(res => {
          this.zsrbn = res.data.data.filter(item =>
            item.reportingType.indexOf("全年")
          ); //半年数据
          this.zsrqn = res.data.data.filter(item =>
            item.reportingType.indexOf("半年")
          ); //全年数据
          this.ZSR = this.zsrbn; //默认显示半年数据
        });
        // 请求收入构成数据
        axios.get("/api/listedCompany06?id=" + newVal.query.id).then(res => {
          this.srgcbn = res.data.data.filter(item =>
            item.reportingType.indexOf("全年")
          ); //半年数据
          this.srgcqn = res.data.data.filter(item =>
            item.reportingType.indexOf("半年")
          ); //全年数据
  ...
  mounted() {
    ...
    // 请求管理模块图表数据
    axios.get("/api/listedCompany03?id=" + this.$route.query.id).then(res => {
      this.glgmqn = res.data.data.filter(item =>
        item.reportingType.indexOf("半年")
      ); //全年数据
      this.glgmbn = res.data.data.filter(item =>
    ...
    // 请求总收入图表数据
    axios.get("/api/listedCompany05?id=" + this.$route.query.id).then(res => {
      this.zsrbn = res.data.data.filter(item =>
        item.reportingType.indexOf("全年")
      ); //半年数据
      this.zsrqn = res.data.data.filter(item =>
    ...
    // 请求收入构成数据
    axios.get("/api/listedCompany06?id=" + this.$route.query.id).then(res => {
      this.srgcbn = res.data.data.filter(item =>
        item.reportingType.indexOf("全年")
      ); //半年数据
      this.srgcqn = res.data.data.filter(item =>
...
.container_body_left_body > div::-webkit-scrollbar {
  display: none;
}
