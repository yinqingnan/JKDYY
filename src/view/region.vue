      ...
      <div class="Left">
        ...
        <div class="Title">
          <ul>
            ...
            <li>
              <h1>{{ list1[0] }}</h1>
              <span v-for="(item, index) in titlename" :key="index"
                >{{ item.nodeAchievingRate }}%</span
              >
            </li>
            <li>
              <h1>{{ list1[1] }}</h1>
              <span v-for="(item, index) in titlename" :key="index"
                >{{ item.nodeCategory1Overdue }}个</span
              >
            </li>
          </ul>
          <ul>
          ...
          <div class="Chart1">
            ...
            <div>
              <div>
                ...
                <div class="xmnhxq">
                  <h2 @click="toxmnhzb">
                    更多 <span class="iconfont iconpub_right"></span>
                  </h2>
                </div>
              </div>
              <div class="xmnh">
  ...
  methods: {
    ...
    selectchange(value) {
      ...
      this.axios.get("api/projectBycId?companyId=" + value).then(res => {
        ...
        let URL =
          "?" + "quid" + "=" + this.value + "&" + "xmid" + "=" + this.xmid;
        ...
        this.axios
          ...
          .then(res => {
            ...
            for (let i in res.data.data[0]) {
              this.xmnhysqn.push(res.data.data[0][i]);
            }
          });
        //项目能耗用水数据月度
        this.axios
        ...
        this.axios
          ...
          .then(res => {
            this.xmnhydqn = [];
            for (let i in res.data.data[0]) {
              this.xmnhydqn.push(res.data.data[0][i]);
            }
          });
        this.axios
        ...
        this.axios
          ...
          .then(res => {
            ...
            this.bssxzzt = [];
            if (this.date >= 7) {
              this.bssxzzt = incomeButtom;
              this.xsyf = 1;
            } else {
              this.bssxzzt = incomeTop;
              this.xsyf = 0;
            }
          });
        ...
        this.axios.get("/api/projectYMRs?projectId=" + this.xmid).then(res => {
          ...
          let objstr = JSON.stringify(obj).replace(/Rate/g, "");
          ...
          if (this.date >= 7) {
            this.zhsflsxzzt = incomeButtom;
            this.xsyf = 1;
          } else {
            this.zhsflsxzzt = incomeTop;
            this.xsyf = 0;
          }
        });
        // 获取综合收费率  横向柱状图数据
        this.axios
          .get("/api/projectRateAvg?projectId=" + this.xmid)
          .then(res => {
            let arr = res.data.data;
            this.zhsflhxzzt = [];
            arr.forEach(element => {
        ...
        this.axios.get("/api/projectPZYM?projectId=" + this.xmid).then(res => {
          ...
          this.xcpzsxzzt = [];
          if (this.date >= 7) {
            this.xcpzsxzzt = incomeButtom;
            this.xsyf = 1;
          } else {
            this.xcpzsxzzt = incomeTop;
            this.xsyf = 0;
          }
        });
        //现场品质横向柱状图
    ...
    selectchange1(value) {
      ...
      //项目能耗用水数据月度
      this.axios
        .get("/api/projectWaterMType?projectId=" + value + "&topcount=2")
        .then(res => {
          let arr = res.data.data;
          this.xmnhdyys = [];
          this.xmnhdyysname = [];
      ...
      // 获取项目能耗用电数据   月度
      this.axios
        .get("/api/projectElectMType?projectId=" + value + "&topcount=2")
        .then(res => {
          let arr = res.data.data;
          this.xmnhyddy = [];
          this.xmnhyddyname = [];
      ...
      //现场品质提示信息
      this.axios.get("/api/projectXCPZtype?projectId=" + value).then(res => {
        // console.log(res.data.data)
        this.xcpzmsg = res.data.data;
      });
    },
